...
bemvindo = ["Bem vindo","Que bom te ver na praia","que bom te ver aqui","ainda bem que você chegou"]

tchau = ["Tchau","Adeus","Até logo","Volte Sempre","Foi embora porque volta","Voltaa"]

class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(19.0 , 0.0 , 13.5 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.send_whisper(user.id,f"🤍{random.choice(bemvindo)} {user.username} utilize !emote list para lista de emotes ou 1 ao 97")
        await self.highrise.send_whisper(user.id,f"[🤖]Aluguel ou Vendas de Arquivo de bot chame [@Shokytoo] no privado")
           
        await self.highrise.send_emote("dance-tiktok9")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if        message.startswith("/tele") or              message.startswith("/tp") or              message.startswith("/fly") or     message.startswith("!tele") or      message.startswith("!tp") or     message.startswith("!fly"):
          if user.username =="Ashokk":            await self.teleporter(message)
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"{random.choice(tchau)}  {user.username}! 👋🏻")
        await self.highrise.send_emote("dance-tiktok9")